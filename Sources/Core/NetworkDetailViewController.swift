    ...
    override func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
                ...
                if let content_ = NSString(cString: cString, encoding: String.Encoding.utf8.rawValue) {
                    ...
                    if self.httpModel?.url.absoluteString.contains(mainHost) == true {
                        ...
                        if #available(iOS 8.2, *) {
                            height = content_.height(with: UIFont.systemFont(ofSize: 14, weight: UIFontWeightHeavy), constraintToWidth: (UIScreen.main.bounds.size.width - 92))
                        } else {
                        ...
                        //计算NSString高度
                        if #available(iOS 8.2, *) {
                            height = content_.height(with: UIFont.systemFont(ofSize: 14, weight: UIFontWeightRegular), constraintToWidth: (UIScreen.main.bounds.size.width - 92))
                        } else {
                            // Fallback on earlier versions
        ...
        if detailModel.image == nil {
            if let content = detailModel.content {
                ...
                //计算NSString高度
                let height = content.height(with: UIFont.systemFont(ofSize: 14), constraintToWidth: (UIScreen.main.bounds.size.width - 30))
                return height + 70
            }
            return 0 //code never go here --by liman
        }
