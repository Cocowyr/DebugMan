                ...
                if let content_ = NSString(cString: cString, encoding: String.Encoding.utf8.rawValue) {
                    ...
                    if self.httpModel?.url.absoluteString.contains(serverHost) == true {
                        ...
                        if #available(iOS 8.2, *) {
                            height = content_.height(with: UIFont.systemFont(ofSize: 17, weight: UIFontWeightBold), constraintToWidth: (UIScreen.main.bounds.size.width - 92))
                        } else {
                        ...
                        //计算NSString高度
                        if #available(iOS 8.2, *) {
                            height = content_.height(with: UIFont.systemFont(ofSize: 12, weight: UIFontWeightMedium), constraintToWidth: (UIScreen.main.bounds.size.width - 92))
                        } else {
                            // Fallback on earlier versions
